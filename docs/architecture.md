...
|---|---|---|---|
| Transactional email — **Resend** *(shipped 2026-08-08)* | OTP delivery only, `core/resend_client.py`. A genuinely undiscovered gap until this date: `environment` defaults to `"local"` and nothing set `ENVIRONMENT=production` on the deployed service, so the dev-only "log the code instead of emailing it" stopgap was silently active in real production the whole time — every OTP for every real signup was written in plaintext to Render's logs, and no email was ever actually sent, Resend or otherwise | User cannot log in; a send failure is swallowed (edgecases.md A1/A2 — must not be distinguishable from success) but logged server-side as `otp_email_send_failed` | No second provider on standby yet — a real gap, not yet built. `send_otp_email` raising `EmailSendFailed` never blocks the HTTP response either way, so a Resend outage degrades to "no one can complete a fresh login," not a 500 |
